...
public class HelpingCouponService {
	 ...
	 */
		...
		public boolean updateCouponData() {
			System.out.println("HelpingCouponService.updateCouponData(): START");
				...
				try {
					...
					if(connection != null && !connection.isClosed()) {
			            ...
			            if(updatedRows > 0) {
			            	...
			            	if(usercouponInfo.getCouponRequired().equalsIgnoreCase("Y")) {
			            		checkPervasiveData();
			            	}
			            	return true;
			            }else {
			...
			private boolean checkPervasiveData() {
				...
				try {
					...
					if(connection != null  && !connection.isClosed()) {
						...
						rs = stmt.executeQuery();
						if(!rs.next()) {
							System.out.println("ServiceCoupon.checkPervasiveData(): DATA DOES NO EXIST IN PERVASIVE TABLES FOR MPID: "+usercouponInfo.getMpid());
							return insertFrysCoba();
						}else {
